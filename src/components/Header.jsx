...
import { getUnreadCount } from "../api/contentApi";
...
function Header({ darkMode, setDarkMode, onLogout, userEmail, onOpenProfile, onSearch }) {
  ...
  // ✅ Define what happens when a notification arrives
  const handleRealTimeNotification = useCallback(() => {
      // Instantly increase the badge count by 1
      setUnreadCount((prev) => prev + 1);
  }, []);

  // ✅ Pass the function to the hook
  useWebSocket(userEmail, handleRealTimeNotification);
  ...
  useEffect(() => {
    if (userEmail) {
        fetchCount();
        // Keep polling just in case, but rely mostly on WebSocket now
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 items-center justify-between gap-4">
            ...
            <Link to="/home" className="flex items-center gap-3 group shrink-0">
              <img src={logo} alt="LearnHub Logo" className="h-10 w-auto object-contain group-hover:scale-105 transition-transform drop-shadow-sm" />
              <div><h1 className="text-xl font-bold text-slate-800 dark:text-slate-100 tracking-tight hidden sm:block">LearnHub</h1></div>
            </Link>
            ...
            <nav className="hidden md:flex items-center gap-1 bg-slate-100 dark:bg-slate-900/50 p-1 rounded-xl border border-slate-200 dark:border-slate-800 shrink-0">
              <Link to="/home" className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all ${isActive("/home")}`}> <FaHome /> Home </Link>
              <Link to="/dashboard" className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all ${isActive("/dashboard")}`}> <FaColumns /> Dashboard </Link>
            </nav>
            ...
            {onSearch && (
              <div className="flex-1 max-w-md hidden lg:block">
                <div className="relative group">
                  ...
                  <input type="text" placeholder="Search contents..." onChange={(e) => onSearch(e.target.value)} className="w-full bg-slate-100 dark:bg-slate-900 border-none rounded-full py-2 pl-10 pr-4 text-sm text-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-sky-500 focus:bg-white dark:focus:bg-black transition-all outline-none placeholder:text-slate-400" />
                </div>
              </div>
            )}
            ...
            <div className="flex items-center gap-2 sm:gap-3 shrink-0">
              ...
              {/* Notification Bell */}
              {userEmail && (
                  <button onClick={() => setNotifOpen(true)} className="relative p-2 rounded-full text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors" title="Notifications">
                      <FaBell className="text-lg" />
                      {/* ✅ This badge updates automatically now! */}
                      {unreadCount > 0 && (
              ...
              {userEmail && (
                <button onClick={onOpenProfile} className="hidden sm:flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-800 px-3 py-1.5 rounded-full border border-slate-200 dark:border-slate-800 transition-all cursor-pointer group">
                  {userAvatar ? (
                      <img src={userAvatar} alt="Profile" referrerPolicy="no-referrer" className="w-6 h-6 rounded-full border border-slate-300 dark:border-slate-600 object-cover" />
                  ) : (
        ...
        {onSearch && (
          <div className="lg:hidden px-4 pb-3 animate-in slide-in-from-top-2">
             <div className="relative">
                ...
                <input type="text" placeholder="Search..." onChange={(e) => onSearch(e.target.value)} className="w-full bg-slate-100 dark:bg-slate-900 rounded-lg py-2 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500 dark:text-white" />
             </div>
          </div>
        )}
      </header>
